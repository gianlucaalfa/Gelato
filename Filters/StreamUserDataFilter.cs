...
) : IAsyncActionFilter
{
    ...
    )
    {
        ...
        // A handled error or an early result is not a successful user-data mutation.
        var status = (executed.Result as IStatusCodeActionResult)?.StatusCode
            ?? ctx.HttpContext.Response.StatusCode;
        if (executed.Exception is not null || executed.Canceled || status is < 200 or >= 300)
            return;
        ...
        try
        {
            if (
                ...
                || libraryManager.GetItemById<BaseItem>(itemId, user)
                    is not Video { PrimaryVersionId: { } primaryId } row
                || !row.HasStreamTag()
                || !(row.GelatoData<List<Guid>>("userIds")?.Contains(user.Id) ?? false)
                || libraryManager.GetItemById<BaseItem>(primaryId, user) is not { } primary
