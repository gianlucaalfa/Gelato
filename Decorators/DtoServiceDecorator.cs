...
) : IDtoService
{
    ...
    )
    {
        ...
        // Jellyfin can filter inaccessible items; positional matching would apply the wrong metadata.
        var byId = items.DistinctBy(item => item.Id).ToDictionary(item => item.Id);
        foreach (var dto in list)
        {
            if (!byId.TryGetValue(dto.Id, out var item)) continue;
            AddPrimaryVersionFields(dto, item, options, user);
            CountStreamsAsOneSource(dto, item);
            Patch(dto, item, true, user);
        }
        return list;
    }
